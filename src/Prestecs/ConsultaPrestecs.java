...
public class ConsultaPrestecs extends JFrame {
    ...
    public ConsultaPrestecs(Usuari usuari) {
        ...
        tableModel = new DefaultTableModel(new Object[]{"Títol", "Autor", "Editorial", "Categoria", "Estat"}, 0);
        table = new JTable(tableModel) {
            @Override
            public Component prepareRenderer(TableCellRenderer renderer, int row, int column) {
                Component c = super.prepareRenderer(renderer, row, column);
                String estat = (String) getValueAt(row, 4);
        ...
        table.getColumnModel().getColumn(4).setPreferredWidth(100); // Estat
    ...
    private void omplirTaulaPrestecs() {
        ...
        try (Connection connection = Connexio.getConnection();
            PreparedStatement statement = connection.prepareStatement(
                    "SELECT l.Títol, l.Autor, l.Editorial, l.Categoria, p.Data_Retorn_Prevista, p.Data_Retorn_Real " +
                            "FROM llibres l " +
                            "INNER JOIN préstecs p ON l.ID_Llibre = p.ID_Llibre " +
                            "WHERE p.ID_Usuari = ?")) {
            // El jorge tendria que estar orgulloso c*llons!!!
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                ...
                while (resultSet.next()) {
                    ...
                    tableModel.addRow(new Object[]{titol, autor, editorial, categoria, estat});
                }
            }
